...
const computeMonthlyTotals = (transactions: TransactionItem[]) => {
  ...
  for (const key in monthlyTotals) {
    if (monthlyTotals.hasOwnProperty(key)) {
      monthlyTotals[key].income = parseFloat(
        monthlyTotals[key].income.toFixed(2)
      );
      monthlyTotals[key].expense = parseFloat(
        monthlyTotals[key].expense.toFixed(2)
      );
    }
  }
...
const computeSpentToday = (
  ...
  const totalSpent = transactions.reduce((sum, transaction) => {
    ...
    if (
      ...
      isSameDay(transaction.date, targetDate)
    ) {
 ...
 */
const computeCurrentMonthTotalSpent = (
  transactions: TransactionItem[]
): number => {
  ...
  const totalSpent = transactions.reduce((sum, transaction) => {
    ...
    if (
      transaction.type === "Expense" &&
      transactionMonthKey === currentMonthKey
    ) {
      return sum + transaction.amount;
    }
    return sum;
...
export const transactionsToCategoryBudgets = (
  ...
  if (range?.startDate || range?.endDate) {
    filteredTransactions = transactions.filter((t) => {
      ...
      const startTimestamp = range.startDate
  ...
  for (const categoryName in categoryMap) {
    const spent = categoryMap[categoryName as any];
...
const BudgetPage = () => {
  ...
  const categoriesData = transactionsToCategoryBudgets(transactions, {
    startDate: startOfMonth(new Date()),
    endDate: new Date(),
  });

  const handleUpdateIncomeSavings = (
  ...
  return (
    <div className="flex flex-col justify-start items-center px-6 py-6 gap-6">
      ...
      />
      <CategoriesSpentCard data={categoriesData} />
    </div>
  );
};
